...
//new plan, go back to old version and just redirect to point in site where props.history would work 
...
//
/* THE BELOW VERSION FETCHES DATA FROM SERVER SO YOU CAN GO TO PAGE DIRECTLY, 
   BUT NEW PLAN IS TO HAVE A REDIRECT TO THE FETCH PAGE IF YOU TRY TO GO HERE DIRECTLY
...
makeCheckBoxes() {
  ...
  if (this.state.initialFarmgood.relationships != undefined ) {
    ...
    //debugger
    if (this.props.FarmgoodFormData.relationships.days.data) {
      //debugger
      console.log(thisWeek)
      console.log(this.state.theWeek)
      var oldDays = this.props.FarmgoodFormData.relationships.days.data.filter(day => {
        for (let i = 0; i< thisWeek.length; i++) {
          //debugger 
          if (day.name === thisWeek[i][0]) { 
            thisWeek[i][1] = true 
  ...
  thisWeek.forEach(day => {
    for (let j = 0; j<thisWeek.length; j++) {
      if (thisWeek[j] != this.state.theWeek[j]) {
        changed = true
      }
    }
  })
...
toggleCheckbox = (event) => {
  ...
  let days_array
  //debugger 
  if (this.props.FarmgoodFormData.relationships["days-available"].data == undefined) {
    
    debugger  //THIS IS FOR FGS THAT WERE CREATED BEFORE DAYS-AVAILABLE WAS REQUIRED, SHOULDN'T REALLY GET HERE
  ...
  if (this.selectedCheckboxes.has(event[0])) {
    ...
    //this.props.FarmgoodFormData.relationships.days.data
    if (this.props.FarmgoodFormData.relationships.days_array) {
      ...
      debugger
      days_array = this.props.FarmgoodFormData.relationships.days.data.filter(day => day.name !== event[0])
    }
    ...
    if (this.props.FarmgoodFormData.relationships.days_array) {
      ...
      //debugger 
      days_array = this.props.FarmgoodFormData.relationships["days-available"].data.concat(event[0]) 
